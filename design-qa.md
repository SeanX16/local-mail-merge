...
- state: 演示数据；字段管理和“目标岗位”筛选同时展开；另行验收了选中警告记录后的邮件预览、创建确认框和设置中的分级规则页。
...
- Copy and content: 主界面使用“可创建（含警告）/无警告/有警告/硬拦截”的分级文案。每个问题在预览中明确标记“警告”或“拦截”，创建前再汇总警告数量。
- Accessibility and states: 使用原生按钮、输入框、选择框和对话框语义；警告记录默认不勾选但用户可主动勾选；硬拦截记录不可勾选；筛选全选支持 indeterminate；主操作有二次确认。
...
- 设置框是本轮新增状态，原设计稿没有展开的设置页；因此以已批准 A2 的字体、色彩、间距、图标和表面处理作为视觉约束，并分别检查模板、Outlook、导入与安全三个状态。
...
- primary interactions tested: 字段管理打开/关闭、显示字段增减、目标岗位筛选打开/应用、点击行更新邮件预览、模板下拉框加载、设置打开/切换三页/关闭、已拦截原因显示、创建草稿确认框打开/关闭。
...
- [x] 完成开发态、打包态、模板目录、Core 回归和视觉对比验收。
